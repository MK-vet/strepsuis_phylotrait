...
"""
Tests for evolutionary metrics in EvolutionaryAnalysis class.

Tests beta diversity, evolution rates, and other evolutionary metrics.
"""
...
class TestEvolutionaryMetrics:
    """Test evolutionary metrics calculations."""
    ...
    def test_calculate_evolution_rates_zero_nodes(self):
        ...
        assert isinstance(result, pd.DataFrame)
        if not result.empty:
            # First cluster should have rate 0.0
            assert result.iloc[0]['EvolutionRate'] == 0.0
    ...
    def test_evolutionary_cluster_analysis(self, sample_tree):
        ...
        if not result.empty:
            ...
            assert 'MeanPairwiseDist' in result.columns
